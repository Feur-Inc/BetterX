export function createEmojiPreview(emoji) {
    ...
    function attachInputEvent() {
        ...
        if (tweetInput) {
            ...
            // Add key events to detect deletions and update preview accordingly
            tweetInput.removeEventListener('keyup', handleInputForPreview);
            tweetInput.addEventListener('keyup', handleInputForPreview);
            tweetInput.removeEventListener('keydown', function(e) {
                // Handle backspace and delete keys specifically
                if (e.key === 'Backspace' || e.key === 'Delete') {
                    // Use setTimeout to ensure we check after the deletion occurred
                    setTimeout(handleInputForPreview, 0);
                }
            });
